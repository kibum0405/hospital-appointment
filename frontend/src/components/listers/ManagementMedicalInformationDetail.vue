<template>
    <v-card outlined>
        <v-card-title>
            MedicalInformation # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="Diagnosis" v-model="item.diagnosis" :editMode="editMode" @change="change" />
            </div>
            <div>
                <String label="Treatment" v-model="item.treatment" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/medicalInformations/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
